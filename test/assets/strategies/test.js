...
define(['taoItems/assets/manager', 'taoItems/assets/strategies'], function(assetManagerFactory, strategies) {
    ...
    var packedUrlDataProvider = [
        {
            ...
            assets: {
                img: {
                    'http://my.cdn/my/test.png': 'http://my.cdn/my/test.png?signed=true',
                    'http://my.cdn/my/test2.png': 'http://completely.different/url/test.png'
                },
                css: { 'http://my.cdn/my/test2.css': 'http://completely.different/url/test.css' }
            }
        },
        {
    ...
    QUnit.cases.init(packedUrlDataProvider).test('resolve ', function(data, assert) {
    ...
    QUnit.cases.init(baseUrlDataProvider).test('resolve ', function(data, assert) {
    ...
    QUnit.cases.init(externalDataProvider).test('resolve ', function(data, assert) {
    ...
    QUnit.cases.init(base64DataProvider).test('resolve ', function(data, assert) {
    ...
    QUnit.cases
        ...
        .test('resolve ', function(data, assert) {
            ...
            if (!resolved) {
                ...
                assert.equal(
                    /^bust=.*$/.test(resolved.split('?')[1]),
                    data.bust,
                    'The resolved url has the expected buster'
                );
            }
        });
});
